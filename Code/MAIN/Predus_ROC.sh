...
do
   echo $i
   echo "Protein" "Threshold" "TP" "FP" "N" "Neg" "TPR" "FPR" >> /Users/evanedelstein/Desktop/LAB/Raji_Summer2019_atom/Code/Data_files/ROC_scores/P_score_.${i}
      ...
      do
        ...
        cat "$file" | awk '{print $3, $4}'| awk -v p=$i '{if($2 >= p) {print $1}}' >> ${combined_file}.${i}
        ...
        Surf=`cat "$Surface_predictions" | grep '0.' | tail -n +2 | awk '{if($4 >= 0.05) {print $1}}' | awk 'END{print NR}'`
        Neg=$((Surf-N))
        echo "Surf = " $Surf "Neg = " $Neg
        TPR=`bc <<<"scale=3; $TP/$N"`
        FPR=`bc <<<"scale=3; $FP/$Neg"`
        echo $protienID $i $TP $FP $N $Neg $TPR $FPR >> /Users/evanedelstein/Desktop/LAB/Raji_Summer2019_atom/Code/Data_files/ROC_scores/P_score_.${i}
       done
    table=/Users/evanedelstein/Desktop/LAB/Raji_Summer2019_atom/Code/Data_files/ROC_scores/P_score_.${i}.table
    cat /Users/evanedelstein/Desktop/LAB/Raji_Summer2019_atom/Code/Data_files/ROC_scores/P_score_.${i} | column -t >> $table
