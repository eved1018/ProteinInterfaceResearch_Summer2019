...
def log_reg_bnch__predus_dockpred():
    ...
    preduscoef = coefficients[1]
    dockpredcoef = coefficients[2]
    val = (coefficients[0] + preduscoef * predusval +  dockpredval*  dockpredcoef)*(-1)
    exponent = np.exp(val)
    pval = (1/(1+exponent))
    results = pd.DataFrame({"residue": protein, "prediction value": pval})
...
def log_reg_nox_predus_ispred():
    ...
    preduscoef = coefficients[1]
    ispredcoef = coefficients[2]
    val = (coefficients[0] + preduscoef * predusval + ispredval* ispredcoef)*(-1)
    exponent = np.exp(val)
    pval = (1/(1+exponent))
    results = pd.DataFrame({"residue": protein, "prediction value": pval})
